...
const COORDINATE_REVERSE_FUNCTIONS = """
// ================== ОБРАТНОЕ ПРЕОБРАЗОВАНИЕ ДЛЯ ЧАСТИЦ ==================
function getCanvasFromLonLat(lon, lat, region, canvas) {
    let mapLeft, mapTop, mapRight, mapBottom, projection;
    ...
    if (region === 'wo') {
        // Простое линейное преобразование для мирового океана
        const x = mapLeft + ((lon + 180) / 360) * (mapRight - mapLeft);
        const y = mapTop + ((90 - lat) / 180) * (mapBottom - mapTop);
        return { x: x, y: y };
    } else {
        // Для полярных регионов нужна проекция через proj4
        let meterCoords;
        if (region === 'arctic') {
            // Преобразование для Арктики
            const point = proj4("EPSG:4326", projection, [lon, lat]);
            meterCoords = point;
        } else if (region === 'antarc') {
            // Преобразование для Антарктики  
            const point = proj4("EPSG:4326", projection, [lon, Math.abs(lat)]);
            meterCoords = point;
        }
        ...
        const centerX = (mapRight - mapLeft) / 2;
        const centerY = (mapBottom - mapTop) / 2;
        const normalizedX = meterCoords[0] / 3329743;
        const normalizedY = meterCoords[1] / 3329743;
        
        const x = mapLeft + centerX + normalizedX * centerX;
        const y = mapTop + centerY - normalizedY * centerY; // Ось Y инвертирована
...
async function loadAndShowParticles() {
    try {
        ...
        const velocityResponse = await fetch('/api/particles/trajectories', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                date: date,
        ...
        const particlesResponse = await fetch('/api/particles/initial', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
class SimpleParticleSystem {
    ...
    // ⭐ ВАЖНО: метод должен быть объявлен правильно
    setProjection(projectionFunc) {
        this.geoToCanvas = projectionFunc;
    }
    
    initialize(seedParticles) {
        ...
        console.log('✅ Загружено ' + this.particles.length + ' частиц');
    }
    
    updateParticles() {
        // Пусто - частицы не двигаются
    }
...
function geographicToPixel(lon, lat, canvas) {
    return window.getCanvasFromLonLat(lon, lat, 
        document.getElementById('regionSelect').value, canvas);
}
...
<script type="module">
    console.log('🔄 Начинаем загрузку модуля частиц...');
    
    async function initParticleSystem() {
        try {
            // Загружаем модуль (теперь путь правильный!)
            const module = await import('/static/js/particle-system/core.js');
            console.log('✅ Модуль ParticleSystem загружен:', module);
            
            // Создаём глобальный экземпляр
            if (typeof module.ParticleSystem !== 'undefined') {
                window.ParticleSystem = module.ParticleSystem;
                
                // Инициализируем сразу при загрузке
                document.addEventListener('DOMContentLoaded', function() {
                    const canvas = document.getElementById('particleCanvas');
                    if (canvas) {
                        window.particleSystem = new module.ParticleSystem('particleCanvas');
                        console.log('🌀 ParticleSystem создан');
                    } else {
                        console.warn('⚠️ Canvas particleCanvas не найден');
                    }
                });
            }
        } catch (error) {
...
window.getCanvasFromLonLat = getCanvasFromLonLat;
...
window.geographicToPixel = function(lon, lat, canvas) {
    const region = document.getElementById('regionSelect').value;
    return getCanvasFromLonLat(lon, lat, region, canvas);
};
"""

# Сборка всего JavaScript кода
const ALL_JAVASCRIPT = MAP_FUNCTIONS * MODAL_FUNCTIONS * COORDINATE_FUNCTIONS * COORDINATE_REVERSE_FUNCTIONS *
                      ...
                      UPDATED_INITIALIZATION_CODE * PARTICLE_FUNCTIONS * PARTICLE_INIT_CODE * PARTICLE_FUNCTIONS_FIXED * PARTICLE_ANIMATION_FUNCTIONS #* JS_MODULE_IMPORTS    
